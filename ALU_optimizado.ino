...
void loop() {
  ...
  // AND
  if ((EstadoA && EstadoB) && (!z && y && !x && w)){
    digitalWrite(13, HIGH); // SALIDA DEL AND
  }
  // OR
  if ((EstadoA || EstadoB) && (!z && y && x && !w)){
    digitalWrite(13, HIGH); // SALIDA DEL OR
  }
  // NOT
  if (!EstadoA && (!z && y && x && w)){
    digitalWrite(13, HIGH); // SALIDA DEL NOT
  }
  // XOR
  if ((EstadoA ^ EstadoB) && (z && !y && !x && !w)){
    digitalWrite(13, HIGH); // SALIDA DEL XOR
  }
  // NOR
  if ((!EstadoA && !EstadoB) && (z && !y && !x && w)){
    digitalWrite(13, HIGH); // SALIDA DEL NOR
  }
  // XNOR
  if ((!EstadoA && !EstadoB || EstadoA && EstadoB) && (z && !y && x && !w)){
    digitalWrite(13, HIGH); // SALIDA DEL XNOR
  }
  // NAND
  if (((!EstadoA && !EstadoB) || (EstadoA ^ EstadoB)) && (z && !y && x && w)){
    digitalWrite(13, HIGH); // SALIDA DEL NAND
  }
